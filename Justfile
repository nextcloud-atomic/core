...
watch-backend:
    HOST=0.0.0.0 PORT=3000 mold -run cargo watch --workdir /workspace/ -w crates/nca-backend -w crates/nca-system-api -w crates/nca-error -w crates/grpc-journal -w public --no-gitignore -x "run --features mock --bin nca-backend"
build:
    ( cd crates/nca-frontend && npm run release && dx bundle -r && rm -rf /workspace/public && cp -a /workspace/target/dx/nca-frontend/release/web/public /workspace/)
